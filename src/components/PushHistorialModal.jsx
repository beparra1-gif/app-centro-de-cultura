    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
          ...
          <button onClick={() => setMostrarHistorialPush(false)} style={{ background: 'rgba(120,120,128,0.10)', border: 'none', fontSize: '20px', cursor: 'pointer', width: '34px', height: '34px', borderRadius: '999px' }}>✕</button>
        </div>
            ...
            {historialPushTotal.slice().reverse().map(push => (
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                  ...
                  {!push.leida && <span style={{ fontSize: '9px', background: 'linear-gradient(180deg, #2f8cff 0%, var(--azul-electrico) 100%)', color: 'white', padding: '2px 6px', borderRadius: '999px', fontWeight: '700' }}>NUEVA</span>}
                </div>
              </div>
            ))}
          </div>
